...
export default function transformer(file: FileInfo, api: API) {
  ...
  const createCalls = root.find(j.CallExpression, {
    callee: {
      type: 'Identifier',
      name: 'create'
    }
  ...
  for (const importPath of importDeclaration.paths()) {
    if (needProvide || needMap) {
      ...
      if (needProvide && specifiers && !specifiers.some(spec => spec.local?.name === 'provide')) {
        specifiers.push(j.importSpecifier(j.identifier('provide')));
      }

      if (needMap && specifiers && !specifiers.some((spec): spec is ImportSpecifier => spec.local?.name === 'map')) {
        specifiers.push(j.importSpecifier(j.identifier('map')));
      }

      // Remove 'create' specifier
      if (specifiers) {
        importPath.value.specifiers = specifiers.filter(spec => spec.local?.name !== 'create');
      }
    }
  }
